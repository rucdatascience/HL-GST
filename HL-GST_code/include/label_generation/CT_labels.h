...
public:
	...
	/*running limits*/
	long long int max_byte_size = 1e12;
	...
	/*compute label size*/
	long long int compute_label_byte_size() {
		long long int size = 0;
		size = size + two_hop_info.compute_label_byte_size();  // L includes both tree and core indexes
	...
	void compare_label_byte_size() {
		long long int size1 = 0, size2 = 0, size3 = 0;
		size1 += two_hop_info.compute_label_byte_size();  // L includes both tree and core indexes
		for (auto it = Bags.begin(); it != Bags.end(); it++) {
	...
	void record_all_details(string save_name) {
		...
		outputFile << "max_byte_size=" << max_byte_size << endl;
		...
		outputFile << "compute_label_byte_size()=" << compute_label_byte_size() << endl;
		...
		outputFile << "two_hop_info.max_labal_byte_size=" << two_hop_info.max_labal_byte_size << endl;
		outputFile << "two_hop_info.max_run_time_seconds=" << two_hop_info.max_run_time_seconds << endl;

		outputFile << "two_hop_info.compute_label_byte_size()=" << two_hop_info.compute_label_byte_size() << endl;
...
int lca(CT_case_info& case_info, int x, int y) {
...
long long int extract_distance(CT_case_info& case_info, int source, int terminal) {
	...
	else if (root[source] != root[terminal])  // locate in different trees, have not used lamma 9 here yet
	{
		...
		for (int i = 0; i < r_s_size; i++) {
			int u = L[source][i].vertex; // L[source]×î³õÊÇrootµÄinterface
	...
	else  // locate in same tree
	{
		...
		unordered_map<int, int> Bk_source_dis;
...
void extract_shortest_path(CT_case_info& case_info, int source, int terminal, vector<pair<int, int>>& path) {
