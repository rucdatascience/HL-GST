...
#include <label_generation/CT_labels.h>

/*global values*/
graph_v_of_v<int> global_ideal_graph_CT, global_eliminated_edge_predecessors; // weight of global_eliminated_edge_predecessors is predecessor of a merged edge; (it is slightly faster to use graph_v_of_v<int> than to use map)
...
void CT(graph_v_of_v<int>& input_graph, CT_case_info& case_info) {
	...
	for (int i = 1; i <= N; i++) {
		...
		auto& adj_temp = global_ideal_graph_CT[v_x];  // global_ideal_graph_CT is G_i-1
		int v_adj_size = adj_temp.size();
		for (int j = 0; j < v_adj_size; j++) {
			//if (adj_temp[j].second > TwoM_value) { // this causes errors, since each bag in a tree contains same interfaces with different distances
			//	cout << "xx" << endl;
			//	continue;
			//}
			Bags[v_x].push_back({ adj_temp[j].first, adj_temp[j].second });  // Bags[v_x] stores adj vertices and weights of v_x
			int pred = adj_temp[j].first;
			bag_predecessors[v_x].push_back(adj_temp[j].first); // bag_predecessors[v_x] stores predecessors (in merged graphs) for vertices in Bags[v_x]
		}
		...
		for (int j = 0; j < v_adj_size; j++) {
			...
			for (int k = j + 1; k < v_adj_size; k++) {
				...
				int pos = sorted_vector_binary_operations_search_position(global_ideal_graph_CT[adj_j], adj_k);
		...
		// delete edge of v_x and update degree (due to added edges above and deleted edge below)
		for (int j = 0; j < v_adj_size; j++) {
			int m = adj_temp[j].first;
			//update degree
			nd.vertex = m;
			nd.degree = global_ideal_graph_CT[m].size() - 1;  // v_x will be removed from global_ideal_graph_CT[nd.vertex] below
			q.push(nd);
			// remove v_x
			int pos = sorted_vector_binary_operations_search_position(global_ideal_graph_CT[m], v_x);
	...
	vector<vector<two_hop_label>> L1(N);  // Labels for CT-tree index merge later, otherwise may increase query time on PLL
	vector<int> fa(N);
	root.resize(N);
	...
	for (int i = bound_lambda; i >= 1; i--) {
		...
		else  //a non-root in the forest
		{
			...
			{                                                  // add interface
				...
				temp_dis[Bags[root[v_x]][j].first] = std::numeric_limits<int>::max();  // initial dis
			}
	...
	if (case_info.use_P2H_pruning) {
		...
		std::vector<std::vector<pair<int, int>>> ZBags(N);  // bag nodes of decomposied tree
	...
	case_info.time_P2H_pruning = std::chrono::duration_cast<std::chrono::nanoseconds>(end_p2h - begin_p2h).count() / 1e9;
	//------------------------------------------------------------------------------------------------------
	...
	case_info.two_hop_info.max_run_time_seconds = case_info.max_run_time_seconds - to_date_time;
	...
	long long int to_date_byte_size = case_info.compute_label_byte_size();
	case_info.two_hop_info.max_labal_byte_size = case_info.max_byte_size - to_date_byte_size;
	if (case_info.two_hop_info.max_labal_byte_size < 0) {
	...
	if (case_info.d == 0) {
		...
		auto new_instance_graph = graph_v_of_v_update_vertexIDs_by_degrees_large_to_small(global_ideal_graph_CT, vertexID_new_to_old); // sort vertices
		...
		for (int v_k = 0; v_k < N; v_k++) { // it seems that parallel computing is slower
			results.emplace_back(
				pool.enqueue([&old_L, &L, v_k, &vertexID_new_to_old] { // pass const type value j to thread; [] can be empty

					for (auto& xx : L[v_k]) {
						xx.vertex = vertexID_new_to_old[xx.vertex];
						xx.parent_vertex = vertexID_new_to_old[xx.parent_vertex];
					}
					sort(L[v_k].begin(), L[v_k].end(), compare_two_hop_label_small_to_large);
		...
		/*info*/
		case_info.two_hop_info.label_size_before_canonical_repair = new_two_hop_case_info.label_size_before_canonical_repair;
		case_info.two_hop_info.label_size_after_canonical_repair = new_two_hop_case_info.label_size_after_canonical_repair;
		case_info.two_hop_info.canonical_repair_remove_label_ratio = new_two_hop_case_info.canonical_repair_remove_label_ratio;
		case_info.two_hop_info.time_initialization = new_two_hop_case_info.time_initialization;
		case_info.two_hop_info.time_generate_labels = new_two_hop_case_info.time_generate_labels;
		case_info.two_hop_info.time_sortL = new_two_hop_case_info.time_sortL;
		case_info.two_hop_info.time_canonical_repair = new_two_hop_case_info.time_canonical_repair;
		case_info.two_hop_info.time_total = new_two_hop_case_info.time_total;
		case_info.two_hop_info.max_labal_byte_size = new_two_hop_case_info.max_labal_byte_size;
		case_info.two_hop_info.max_run_time_seconds = new_two_hop_case_info.max_run_time_seconds;
	}
	...
	for (int v_k = 0; v_k < N; v_k++) {
		if (L1[v_k].size() > 0) { // direcly removing >=2M tree labels here causes errors, since Bags should also be changed?
	...
	for (int u = 0; u < N; u++) {
		results.emplace_back(
			pool.enqueue([&L, u] {
				for (auto& xx : L[u]) {
					while (1) {
						auto it = global_eliminated_edge_predecessors.edge_weight(u, xx.parent_vertex);
						if (it == std::numeric_limits<int>::max()) {
							break;
						}
						xx.parent_vertex = it;
					}
					/*it is too slow to use CT_extract_distance to update predecessors in large graphs*/
				}
				return 1;
				}));
	}
