...
#include <label_generation/hop_constrained_two_hop_labels.h>
...
void HSDL_thread_function(int v_k) {
	...
	while (Q.size() > 0) {
		...
		if (P_u < query_v_k_u || query_v_k_u == 0) { // query_v_k_u == 0 is to start the while loop by searching neighbors of v_k
			...
			for (auto& xx : ideal_graph_599[u]) {
				...
				if (!global_use_2M_prune) {
					...
					if (yy.second <= node.distance) { // adj_v has been reached with a smaller distance and the same hop
						continue;
					}
					if (dist_hop[adj_v].first == std::numeric_limits<int>::max()) { // adj_v has not been reached
						yy = { Q.push({node}), node.distance };
						Q_handle_priorities_changes.push_back({ adj_v, node.hop });
						...
						if (node.distance < dist_hop[adj_v].first) { // adj_v has been reached with a larger distance
							if (yy.second != std::numeric_limits<int>::max()) {
								Q.update(yy.first, node);
								yy.second = node.distance;
							}
						...
						else if (node.hop < dist_hop[adj_v].second) { // adj_v has been reached with a larger distance and a larger hop
							if (yy.second != std::numeric_limits<int>::max()) {
								Q.update(yy.first, node);
								yy.second = node.distance;
							}
...
void hop_constrained_two_hop_labels_generation(graph_v_of_v<int>& input_graph, hop_constrained_case_info& case_info) {
	...
	max_labal_size_599 = case_info.max_byte_size / sizeof(hop_constrained_two_hop_label);
