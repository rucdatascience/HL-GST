...
void PLL_dij_function(int v_k, graph_v_of_v<int>& input_graph) {
	...
	while (Q.size()) {
		...
		for (auto xx : L_temp_595[u]) {
			...
			if (query_v_k_u > dis) { query_v_k_u = dis; common_hub_for_query_v_k_u = xx.vertex; }
		} //求query的值
...
vector<vector<two_hop_label>> sortL(int num_of_threads) {
...
void PLL(graph_v_of_v<int>& input_graph, two_hop_case_info& case_info) {
	...
	labal_num_595 = 0;
	max_labal_num_595 = case_info.max_labal_byte_size / sizeof(two_hop_label);
