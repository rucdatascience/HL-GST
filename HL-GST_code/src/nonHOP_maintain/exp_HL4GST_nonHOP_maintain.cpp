...
pair<vector<edge_change>, vector<edge_change>> select_decrease_increase_weights(graph_v_of_v<int>& input_graph, int batch_N) {
	...
	/*increase_edges*/
	if (1) { // randomly deleting sparse mock and non-mock edges
		vector<pair<int, int>> edge_pool;
		for (int i = 0; i < V; i++) {
			for (auto adj : input_graph[i]) {
				if (i < adj.first && min(input_graph[i].size(), input_graph[adj.first].size()) < 5) { // 10 makes 2023algo too slow
					edge_pool.push_back({ i, adj.first });
				}
			}
		}
	...
	if (0) { // only removing mock edges is extremely fast (no query in maintenance is needed)
		vector<int> non_mocks, mocks;
		for (int i = 0; i < V; i++) {
			if (is_mock[i]) {
				mocks.push_back(i);
...
void exp_element_nonHOP(string data_name, string file_name, int thread_num, int iteration_num) {
	...
	string save_name = "expHL4GST_nonHOP_maintain_" + file_name + "_" + ec_type_name + "_thread_num_" + to_string(thread_num) + ".csv";
...
void main_exp() {
	...
	if (1) {
		for (int i = 0; i < used_datas.size(); i++) {
			exp_element_nonHOP(used_datas[i], used_datas[i], 80, 30);
		}
	}
}
...
int main(int argc, char* argv[])
{
	...
	if (argc == 5) { // argv[0] is the name of the exe file  e.g.: ./A musae musae 50 3600
		...
		exp_element_nonHOP(argv[1], argv[2], 80, atoi(argv[3]));
	}
	else {
		generate_L_PPR();
	}
